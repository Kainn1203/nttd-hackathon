...
export async function POST(req: NextRequest) {
  ...
  if (typeof body?.userId !== 'number')
    return new Response(JSON.stringify({ error: 'userId (number) is required' }), { status: 400 });
  if (!Array.isArray(body.answers) || body.answers.length !== 10)
    return new Response(JSON.stringify({ error: 'answers must be length 10' }), { status: 400 });
  if (!body.answers.every(v => v === 1 || v === 2 || v === 3))
    return new Response(JSON.stringify({ error: 'each answer must be 1|2|3' }), { status: 400 });
  ...
  const { data, error } = await supabase
    ...
    .insert({ user_id: body.userId, scores: percent, diagnosis_result: typeNames [typeKey] })
    .select()
    .single();
