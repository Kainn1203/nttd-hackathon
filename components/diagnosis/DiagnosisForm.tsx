...
export default function DiagnosisForm({
  ...
  const [shuffledQuestions, setShuffledQuestions] = useState<
    {
      id: number;
      text: string;
      options: { text: string; value: 1 | 2 | 3 }[];
    }[]
  >([]);
  ...
  const completeDiagnosis = async (finalAnswer: 1 | 2 | 3) => {
    ...
    try {
      ...
      const answersArray: (1 | 2 | 3)[] = Array.from(
        { length: 10 },
        (_, i) => allAnswers[i]
      );
  ...
  return (
    <div className="h-screen bg-gradient-to-b from-cyan-300 via-sky-200 to-purple-300 relative overflow-hidden flex items-center justify-center">
      {/* 背景パターン */}
      <div className="absolute inset-0">
      ...
      <div className="w-full max-w-2xl mx-auto px-4 sm:px-6 relative z-10">
        <div className="bg-gradient-to-br from-sky-300/60 via-purple-400/50 to-pink-500/60 backdrop-blur-xl rounded-2xl sm:rounded-3xl shadow-lg border border-white/25 p-3 sm:p-5 lg:p-6">
          {/* ヘッダー */}
          <div className="flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-0 mb-4">
            <button
              onClick={onBack}
              className="text-white/90 hover:text-white text-sm sm:text-base font-semibold drop-shadow transition-all duration-200 hover:scale-105 order-1 sm:order-none"
            >
              ← 戻る
            </button>
            <h2 className="text-base sm:text-lg lg:text-xl font-semibold text-white/90 drop-shadow-lg order-3 sm:order-none">
              質問 {currentQuestion + 1} / {questions.length}
            </h2>
            <div className="bg-white/20 backdrop-blur-sm px-2 py-0.5 sm:px-3 sm:py-1 rounded-full border border-white/30 order-2 sm:order-none">
              <span className="text-white/90 font-semibold drop-shadow text-xs sm:text-sm">
                {userName}さん
              </span>
            </div>
          </div>
          ...
          {/* プログレスバー */}
          <div className="w-full bg-white/20 rounded-full h-2 sm:h-3 border border-white/30">
            <div
              className="bg-gradient-to-r from-cyan-400 to-purple-500 h-2 sm:h-3 rounded-full transition-all duration-500 shadow-sm"
              style={{ width: `${progress}%` }}
            ></div>
          </div>
          <div className="text-right font-semibold text-white/80 text-xs sm:text-sm drop-shadow mb-3">
          ...
          <div>
            <div className="text-center mb-4 sm:mb-6">
              <div className="text-2xl sm:text-3xl lg:text-4xl mb-2 sm:mb-3 drop-shadow-lg">
                {
                  ["💼", "🍻", "👥", "😴", "⏰", "💭", "👥", "⚖️", "🚀", "📱"][
                    currentQuestion
                  ]
              ...
              <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-white/95 leading-relaxed drop-shadow-lg max-w-xl mx-auto px-2 sm:px-0">
              ...
              {shuffledQuestions[currentQuestion].options.map(
                (opt: { text: string; value: 1 | 2 | 3 }, idx: number) => (
                  <button
                    ...
                    className="w-full text-left bg-white/20 hover:bg-white/30 active:bg-white/40 border border-white/30 rounded-xl px-3 sm:px-4 py-2 sm:py-2.5 lg:py-3 transition-all duration-150 backdrop-blur drop-shadow-sm"
                  >
