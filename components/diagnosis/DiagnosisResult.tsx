...
export default function DiagnosisResults({ userName, result, onRestart }: DiagnosisResultsProps) {
  ...
  const typeConfig = {
    type1: {
      ...
      imagePath: '/images/result_score_v1_transparent.png',
      ...
      details: {
        ...
        weakness: '周囲から「やる気がない？」と誤解されることも。チームプレイより個人プレイを重視しがち。',
        advice:
    ...
    type2: {
      ...
      imagePath: '/images/result_score_v2_transparent.png',
      ...
      details: {
        ...
        weakness: '柔軟性に欠けると「冷たい人」と思われることも…。突発的な残業対応が苦手。',
        advice:
    ...
    type3: {
      ...
      imagePath: '/images/result_score_v3_transparent.png',
      ...
      details: {
        ...
        weakness: '効率にこだわりすぎて、人間関係のしがらみや「情」に弱い。人によってはドライに見えてしまうことも…。',
        advice:
    ...
    type4: {
      ...
      imagePath: '/images/result_score_v4_transparent.png',
      ...
      details: {
        ...
        weakness: '頑張りすぎて心身を削ることも。周囲の期待に応えようとして、自分を後回しにしがち。',
        advice:
    ...
    type5: {
      ...
      imagePath: '/images/result_score_v5_transparent.png',
      advice:
        '働くことが生きがい！自己犠牲すらも厭わぬ使命感タイプ',
      details: {
        characteristic:
          '残業や休日出勤を「頑張りの証」と捉える。チームのためなら自分を犠牲にすることもいとわない。',
        strengths:
          '圧倒的な責任感と根性。どんな状況でも踏ん張れる。周囲から「頼れる人」と見られる。',
        weakness: '自分を追い込みすぎて燃え尽きる危険性大。ワークライフバランスは崩れやすい。',
        advice:
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-cyan-300 via-sky-200 to-purple-300 relative overflow-hidden">
      ...
      {/* ▼▼ 枠を一回り小さく（幅/余白/角丸/縮尺） ▼▼ */}
      <div className="max-w-2xl mx-auto px-4 py-8 relative z-10 scale-90">{/* 旧: max-w-4xl px-6 py-12 */}
        <div className="bg-gradient-to-br from-sky-300/60 via-purple-400/50 to-pink-500/60 backdrop-blur-xl rounded-2xl shadow-lg border border-white/25 p-6">{/* 旧: rounded-3xl p-10 */}
         
          {/* ヘッダー（文字は見やすく太めのまま） */}
          <div className="text-center mb-8">{/* 旧: mb-12 */}
            <h1 className="text-4xl font-semibold text-white mb-4 tracking-wide drop-shadow-2xl">{/* 旧: text-5xl mb-6 */}
              診断結果
            </h1>
            <p className="text-lg font-semibold text-white/90 drop-shadow-lg">{/* 旧: text-xl */}
              {userName}さんの社畜診断が完了しました！
            </p>
            <p className="text-white/90 font-semibold mt-2 drop-shadow">{/* 旧: mt-3 */}
          ...
          {/* メイン画像セクション */}
          <div className="text-center mb-8">{/* 旧: mb-12 */}
            <div className="relative inline-block w-full max-w-3xl mx-auto">{/* 旧: max-w-4xl */}
              <div className={`absolute -inset-4 bg-gradient-to-r ${config.color} rounded-3xl blur-2xl opacity-25`}></div>{/* 旧: -inset-7 */}
              <div className={`absolute -inset-3 bg-gradient-to-r ${config.color} rounded-3xl blur-xl opacity-15`}></div>{/* 旧: -inset-5 */}
              <div className="relative bg-white/90 backdrop-blur-sm rounded-3xl p-5 shadow-2xl border-2 border-white/60">{/* 旧: p-6 */}
                <img
                  src={config.imagePath}
                  alt={result.type.name}
                  className="block rounded-3xl object-contain w-full h-auto max-w-3xl mx-auto"/* 旧: max-w-5xl */
                  style={{ aspectRatio: '965/544' }}
                />
              </div>
              <div className={`absolute -top-5 -right-5 w-14 h-14 bg-gradient-to-r ${config.color} rounded-full shadow-xl opacity-90`}></div>{/* 旧: w-16 h-16 */}
              <div className={`absolute -bottom-5 -left-5 w-10 h-10 bg-gradient-to-r ${config.color} rounded-full shadow-lg opacity-70`}></div>{/* 旧: w-12 h-12 */}
              <div className={`absolute top-10 -left-7 w-8 h-8 bg-gradient-to-r ${config.color} rounded-full shadow-lg opacity-50`}></div>{/* 旧: w-10 h-10 */}
            </div>
          </div>


          {/* 結果表示ブロック */}
          <div className={`p-6 bg-gradient-to-r ${config.bgGradient} backdrop-blur-sm rounded-2xl ${config.borderColor} border shadow-lg mb-8`}>{/* 旧: p-8 rounded-3xl mb-10 */}
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between text-center lg:text-left">
              <div className="mb-6 lg:mb-0 lg:mr-10">{/* 旧: mb-8 mr-12 */}
                <h2 className="font-semibold text-white/95 text-4xl drop-shadow-lg mb-4">{/* 旧: text-5xl mb-6 */}
                  {result.type.name}
                </h2>
                <p className="text-xl text-white/85 drop-shadow-sm leading-relaxed">{/* 旧: text-2xl */}
                  {config.advice}
                </p>
              </div>
              ...
              <div className="text-center lg:text-right">
                <div className="text-base text-white/90 font-medium mb-3">{/* 旧: text-xl mb-4 */}
                  スコア
                </div>
                <div className="relative">
                  <div className="text-6xl font-light text-white/95 mb-5 drop-shadow-lg">{/* 旧: text-7xl mb-6 */}
                    {result.scores}
                    <span className="text-2xl text-white/70 ml-2">{/* 旧: text-3xl */}
                      点
                    </span>
                  </div>
                  <div className="w-48 mx-auto lg:mx-0">{/* 旧: w-56 */}
                    <div className="bg-white/20 rounded-full h-6 border border-white/30 overflow-hidden">{/* 旧: h-8 */}
                      <div
                        className={`bg-gradient-to-r ${config.color} h-6 rounded-full shadow-sm transition-all duration-1000 relative`}
                        style={{ width: `${result.scores}%` }}
                      >
                        <div className="absolute inset-0 bg-white/20 rounded-full animate-pulse"></div>
                      </div>
                    </div>
                    <div className="flex justify-between text-white/60 font-semibold text-xs mt-2">{/* 旧: text-sm mt-3 */}
                      <span>0</span>
                      <span>100</span>
                    </div>
                  </div>
          ...
          {/* 詳細分析セクション（外観はそのまま） */}
          <div className="bg-gradient-to-r from-white/15 via-white/10 to-white/5 backdrop-blur-sm rounded-2xl p-8 mb-8 border border-white/30">{/* 旧: mb-10 */}
