...
interface CommunityWithMembers extends Community { //基本のCommunity型に追加情報を付けた型
  member_count?: number; //メンバー数
  is_member?: boolean; //参加中かどうか
}

interface CommunityCardProps { //このコンポーネントが受け取るデータの型
...
export default function CommunityCard({  //上で定義したpropsを受け取る
  community,
  isMember,
  viewMode,
  onCommunityClick,
  ...
  const formatDate = (dateString: string | null) => { // null許容に変更
  ...
  const truncateText = (text: string, maxLength: number) => { //文字切り詰め関数
    ...
    return (
      <div
        className={`bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-200 cursor-pointer transform hover:-translate-y-1 ${
          isMember ? 'border-2 border-blue-200 bg-blue-50/30' : 'border border-gray-200'
        }`} 
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-200 cursor-pointer ${
        isMember ? 'border-l-4 border-blue-500 bg-blue-50/30' : 'border-l-4 border-gray-300'
      }`}
      onClick={handleCardClick}
    >
      <div className="p-4">
